...
export class ExpenseChartComponent {
  @ViewChild('byCategoryChart', { static: true })
  byCategoryChartTemplate: TemplateRef<unknown> | null = null;
  @ViewChild('byDayChart', { static: true }) byDayChartTemplate: TemplateRef<unknown> | null = null;
  @ViewChild('byMonthChart', { static: true }) byMonthChartTemplate: TemplateRef<unknown> | null =
    null;
      ...
      tooltip: {
        callbacks: {
          label: (context) => {
            ...
            const total = Array.isArray(dataset) ? dataset.reduce((sum, val) => sum + val, 0) : 0;
            const percentage = total ? parseFloat(((currentValue / total) * 100).toFixed(1)) : 0;
  ...
  byCategoryChartDatasets: NumberArrayData[] = [];
  ...
  byDayChartDatasets: ChartDataItem[] = [];
  ...
  byMonthChartDatasets: ChartDataItem[] = [];
  ...
  private setChartDataByCategory() {
    this.byCategoryChartLabels = Array.from(
      new Set(this.expenses.map((x) => x.category?.name || '').filter((x) => !!x))
    );
